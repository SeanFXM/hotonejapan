...
import { HomeConfig, HomeContentItem } from "@/types/home-config"
...
export default function HomeConfigPage() {
  ...
  const [activeTab, setActiveTab] = useState<"newProducts" | "supportInfo" | "announcements">("newProducts")
  ...
  const createDefaultConfig = (): HomeConfig => {
    return {
      newProducts: { items: [] },
      supportInfo: { items: [] },
      announcements: { items: [] },
  ...
  const updateContentItem = (id: string, updates: Partial<HomeContentItem>) => {
    if (!config) return
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">主页内容配置</h1>
              <p className="mt-2 text-gray-600">管理主页的标签页内容</p>
            </div>
...
// 内容项编辑器组件（复用品牌配置的编辑器，但使用HomeContentItem类型）
function HomeContentItemEditor({
  item,
  index,
  onUpdate,
  onRemove,
  ...
  index: number
  onUpdate: (updates: Partial<HomeContentItem>) => void
  onRemove: () => void
      ...
      {expanded && (
        <div className="space-y-4">
          ...
          <div>
            ...
            <select
              value={item.type}
              onChange={(e) => onUpdate({ type: e.target.value as HomeContentItem["type"] })}
